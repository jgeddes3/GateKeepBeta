...
// Sub-project 6 task 9: split out of page.tsx (which is now a thin Server
// Component reading the `next` query param, see that file's own comment)
// so the event page's "sign in, then land back here" buy-flow gate has a
// real redirect target to send a signed-out buyer to. `next` is validated
// by the caller (page.tsx's isSafeNext) before it ever reaches this
// component: this file trusts it as a same-origin relative path already.
