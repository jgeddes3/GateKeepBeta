...
interface EventPrivateAddress { address: string; geo: { lat: number; lng: number } | null }

function mapUrl(address: EventPrivateAddress): string {
  const q = address.geo ? `${address.geo.lat},${address.geo.lng}` : address.address;
  return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(q)}`;
}

// The ticket-holder address reveal (spec anatomy: "when signed in and
// users/{uid}/ticketIndex/{eventId} exists, show the exact address block").
...
function useTicketHolderAddress(eventId: string, uid: string | null): EventPrivateAddress | "hidden" {
