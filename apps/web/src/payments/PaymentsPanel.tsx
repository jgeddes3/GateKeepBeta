...
export function PaymentsPanel({ bookingId, uid }: { bookingId: string; uid: string }) {
  ...
  // Hooks run unconditionally, every render, in the same order (the same
  // rule BookingThread itself follows); the early returns below happen AFTER.
  ...
  // own addition, which recomputePaymentSummary doesn't track) rides along
