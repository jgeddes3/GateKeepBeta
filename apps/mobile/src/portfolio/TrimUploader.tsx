...
import { Text, Input, ErrorBanner } from "../ui";
...
export function TrimUploader({ profileId, onDone }: { profileId: string; onDone?: () => void }) {
  ...
  return (
    <View style={{ borderWidth: 1, borderStyle: "dashed", borderColor: tok.border, borderRadius: 8, padding: 12, gap: 8 }}>
      <Text variant="label">Add a track (30-second snippet)</Text>
      <Pressable onPress={() => void pick()} accessibilityRole="button" accessibilityLabel="Choose an audio file"
        style={{ borderWidth: 1, borderColor: tok.border, padding: 10, borderRadius: 8 }}>
        <Text>{picked ? picked.name : "Choose audio file…"}</Text>
      </Pressable>
      <ErrorBanner message={error} />
      {picked && duration > 0 && (
        <>
          <Input placeholder="Track title" value={title} onChangeText={setTitle} maxLength={80} />
          {wholeFileUsed ? (
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
            <Pressable onPress={preview} style={{ borderWidth: 1, borderColor: tok.border, padding: 10, borderRadius: 8 }}>
              <Text>▶ Preview</Text>
            </Pressable>
            <Pressable onPress={() => player.pause()} style={{ borderWidth: 1, borderColor: tok.border, padding: 10, borderRadius: 8 }}>
              <Text>Stop</Text>
            </Pressable>
            <Pressable onPress={() => void upload()} disabled={busy !== null}
              style={{ backgroundColor: tok.accent, padding: 10, borderRadius: 8 }}>
              <Text color={tok.onAccent}>{busy ?? "Upload snippet"}</Text>
            </Pressable>
          </View>
        </>
      )}
