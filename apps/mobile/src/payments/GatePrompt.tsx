...
function WarnBox({ children }: { children: React.ReactNode }) {
  return (
    <View style={{ backgroundColor: "#fef3c7", borderWidth: 1, borderColor: "#fde68a", borderRadius: 8, padding: 12, gap: 8 }}>
      {children}
    </View>
  );
}
...
function CuratorCardGate({ message, curatorProfileId, onRetry }: {
  ...
  return (
    <WarnBox>
      <Text style={{ color: "#92400e" }}>{message}</Text>
      <Pressable onPress={() => setShowSaveCard(true)} style={{ alignSelf: "flex-start", borderWidth: 1, borderColor: "#ddd", borderRadius: 6, paddingVertical: 8, paddingHorizontal: 12 }}>
        <Text>Save a card</Text>
      </Pressable>
    </WarnBox>
  );
}
...
export function GatePrompt({ message, curatorProfileId, viewerIsMusician, onRetry }: {
  ...
  if (message === DEPOSIT_PROCESSING_MESSAGE) {
    return (
      <Text style={{ backgroundColor: "#eff6ff", borderWidth: 1, borderColor: "#bfdbfe", borderRadius: 8, padding: 12, color: "#1e40af" }}>
        {message}
      </Text>
    );
  }
