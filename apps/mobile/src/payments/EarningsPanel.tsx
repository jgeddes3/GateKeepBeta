...
function PendingSettlementsList({ rows }: { rows: PaymentRow[] }) {
  ...
  if (pending.length === 0) return <Text style={{ color: "#666" }}>Nothing pending.</Text>;
  return (
    <View style={{ gap: 6 }}>
      {pending.map((r) => (
...
function HistoryList({ rows }: { rows: PaymentRow[] }) {
  ...
  if (history.length === 0) return <Text style={{ color: "#666" }}>No payout history yet.</Text>;
  return (
    <View style={{ gap: 6 }}>
      {history.map((r) => (
        <Text key={`${r.bookingId}:${r.id}`}>
...
export function EarningsPanel({ profileId }: { profileId: string }) {
  ...
  // `disabled` prop and its `opacity` style read the SAME computed value
  ...
  return (
    <View style={{ gap: 16 }}>
      ...
      {typeof status === "object" && (
        <>
          {status.delinquent && (
            <Text style={{ backgroundColor: "#fee2e2", borderWidth: 1, borderColor: "#fca5a5", borderRadius: 8, padding: 12, color: "#991b1b" }}>
            ...
            <View style={{ gap: 10 }}>
              ...
              <View style={{ gap: 4 }}>
                ...
                <TextInput
                  ...
                  style={{ borderWidth: 1, borderRadius: 8, padding: 8, width: 110 }}
                />
              </View>
              <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap" }}>
                <Pressable onPress={() => void submitPayout("standard")} disabled={payoutBusy}
                  style={{ borderWidth: 1, borderColor: "#ddd", borderRadius: 8, paddingVertical: 10, paddingHorizontal: 14, opacity: payoutBusy ? 0.6 : 1 }}>
                  <Text>Standard (free, 1–3 business days)</Text>
                </Pressable>
                <Pressable onPress={() => void submitPayout("instant")} disabled={instantDisabled}
                  style={{ borderWidth: 1, borderColor: "#ddd", borderRadius: 8, paddingVertical: 10, paddingHorizontal: 14, opacity: instantDisabled ? 0.5 : 1 }}>
                  <Text>{`Instant${previewCents != null && previewFeeCents != null ? ` — fee ${formatCents(previewFeeCents)}` : ""}`}</Text>
                </Pressable>
              </View>
              {instantHint && <Text style={{ fontSize: 12, color: "#666" }}>{instantHint}</Text>}
              {payoutError && (
                <Text style={{ backgroundColor: "#fef3c7", borderWidth: 1, borderColor: "#fde68a", borderRadius: 8, padding: 12, color: "#92400e" }}>
                  {payoutError}
                </Text>
              ...
              {payoutMessage && (
                <Text style={{ backgroundColor: "#f0fdf4", borderWidth: 1, borderColor: "#bbf7d0", borderRadius: 8, padding: 12, color: "#166534" }}>
                  {payoutMessage}
                </Text>
              )}
            </View>
          ...
          <View style={{ gap: 6 }}>
            <Text style={{ fontSize: 16, fontWeight: "700" }}>Pending settlements</Text>
            <PendingSettlementsList rows={rows} />
          </View>
          <View style={{ gap: 6 }}>
            <Text style={{ fontSize: 16, fontWeight: "700" }}>History</Text>
