...
// Mobile twin of apps/web/src/search/useSearch.ts (Task 8, Step 1): same
// contract, same behaviour, byte-for-byte the same state machine. Only the
// import paths differ (this file's own searchApi.ts, which itself routes
// through lib/callable.ts's getFirebase-backed callFn).
...
// every face that renders the "Near me" chip (FilterChips) also has this
...
// chip/select tap should feel immediate), and a page cursor kept in a
...
// itself changes, the same synchronous-during-render idiom the web hook
// uses) or loadMore's own click handler. Neither is inside a useEffect
// body, so runFetch itself never calls setState outside a promise
// callback.
export function useSearch(face: SearchFace, opts: {
  ...
  // later from a press handler with no cancel() of its own kept around,
  ...
  // the deck/discover screens use): the seqRef check alone only guards
  // against a NEWER request superseding this one while the hook is still
  // mounted, it says nothing about the hook itself having unmounted while
  // this promise was still in flight, which would otherwise call setState
  // on a component that's gone. mountedRef is the same "gone" check but
  // for loadMore's own request, which has no per-call cancel() kept
  // around anywhere for an unmount cleanup to call.
