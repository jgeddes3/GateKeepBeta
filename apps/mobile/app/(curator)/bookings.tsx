...
export default function Bookings() {
  ...
  if (!user || !profileId || !profile) {
    return <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
      <Text>{!user || !profileId ? "Switch to a curator profile to see its bookings." : "Loading…"}</Text></View>;
  }
  ...
  if (profile.status !== "approved") {
    return <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 24 }}>
      <Text style={{ textAlign: "center", color: "#666" }}>
        Your curator profile must be approved before it can have bookings.
      </Text></View>;
  }
