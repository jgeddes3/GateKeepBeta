...
metro area â€” team-approved musician/curator profiles, gig booking, and (later) ticketing â€” built
on a shared Firebase backend behind default-deny Firestore rules and Cloud Functions-only
privileged writes.

This repo now spans three sub-projects. **Sub-project 1: Foundation** â€” accounts, auth, profile
lifecycle (draft â†’ review â†’ approve), the mobile + web app shells, an admin approval dashboard,
and notification plumbing. **Sub-project 2: Musician Portfolio** â€” bio/photos/genres/links,
10Ã—30s reviewed audio snippets with server-side trim/transcode, curator-gated booking rates &
preferences, and server-rendered public portfolio pages at `/@handle`, on both mobile and web.
**Sub-project 3: Curator Profiles & Gig Postings** â€” the curator side of the same profile system
...
search. **Sub-project 4: Booking Flow** â€” curators and musicians book each other through either
...
â”œâ”€â”€ firebase.json                 # emulators (incl. storage :9199), functions, firestore config
â”œâ”€â”€ .firebaserc                   # default Firebase project id
â”œâ”€â”€ firestore.rules               # default-deny + narrow allows
â”œâ”€â”€ firestore.indexes.json
â”œâ”€â”€ storage.rules                 # Storage security rules (staging/review/public paths)
â”œâ”€â”€ packages/shared/              # @gatekeep/shared: types + validation, single source of truth
â”‚   â””â”€â”€ src/{types,validation,storagePaths,index}.ts
â”œâ”€â”€ functions/                    # Cloud Functions (v2 callables + triggers)
â”‚   â”œâ”€â”€ src/index.ts              # exports all functions
â”‚   â”œâ”€â”€ src/authTriggers.ts       # onUserCreated â†’ users doc
â”‚   â”œâ”€â”€ src/guards.ts             # requireAuthUid, requireVerifiedEmail, requireProfileMember, requireMusicianProfile, requireCuratorProfile
â”‚   â”œâ”€â”€ src/profiles.ts           # createProfileDraft, submitProfileForReview, deleteProfile
â”‚   â”œâ”€â”€ src/review.ts             # reviewProfile, grantAdmin, audit logging, curator-unpublish takedown cascade
â”‚   â”œâ”€â”€ src/members.ts            # inviteMember, respondToInvite, revokeInvite, removeMember, transferAdmin
â”‚   â”œâ”€â”€ src/account.ts            # deleteAccount
â”‚   â”œâ”€â”€ src/notifications.ts      # push token helpers, notifyUser, approval trigger
â”‚   â”œâ”€â”€ src/portfolio.ts          # updatePortfolio, updateBookingInfo
â”‚   â”œâ”€â”€ src/tracks.ts             # createTrack, updateTrack, deleteTrack, reorderTracks, reviewTrack
â”‚   â”œâ”€â”€ src/media.ts              # processUpload trigger: ffmpeg transcode + sharp photo resize (musician + curator gallery)
â”‚   â”œâ”€â”€ src/curator.ts            # updateCuratorProfile, removeCuratorPhoto (sub-project 3)
â”‚   â”œâ”€â”€ src/geocode.ts            # geocode(address) adapter interface + Stub/Google providers (sub-project 3)
â”‚   â”œâ”€â”€ src/gigs.ts               # createGig, updateGig, publishGig, cancelGig, takedownGig (sub-project 3)
â”‚   â”œâ”€â”€ src/gigSeries.ts          # createSeries, updateSeries, pauseSeries, endSeries (sub-project 3)
â”‚   â”œâ”€â”€ src/scheduled.ts          # runDailySweep + dailySweep onSchedule wrapper (sub-project 3 â€” see Gigs & series below)
â”‚   â”œâ”€â”€ src/adminTools.ts         # searchUsersByName, backfillDisplayNameLower, flagAccount (sub-project 3)
â”‚   â”œâ”€â”€ src/storage.ts            # Storage bucket helper + STORAGE_BUCKET
â”‚   â””â”€â”€ test/*.test.ts            # emulator integration tests
â”œâ”€â”€ apps/mobile/                  # Expo (expo-router): app/, src/lib/firebase.ts, src/auth/, src/shell/
â”‚   â”œâ”€â”€ eslint.config.js          # flat config (Expo's `eslint-config-expo` default)
â”‚   â”œâ”€â”€ src/shell/AccountScreen.tsx # shared account screen; app/(fan|musician|curator)/account.tsx are thin wrappers
â”‚   â”œâ”€â”€ src/portfolio/            # RN portfolio editor: forms, TrimUploader, TrackManager
â”‚   â”œâ”€â”€ src/curator/              # RN curator wizard/editor forms (sub-project 3)
â”‚   â”œâ”€â”€ src/gigs/                 # RN gig composer + series management (sub-project 3)
â”‚   â”œâ”€â”€ app/(curator)/            # curator tab group: dashboard, events (gig composer/list/series), account
â”‚   â””â”€â”€ app/artist/[handle].tsx   # native public portfolio view
â”œâ”€â”€ apps/web/                     # Next.js (App Router): app/, src/lib/firebase.ts, app/admin/ (claim-gated)
â”‚   â”œâ”€â”€ app/join/                 # musician + curator onboarding wizard
â”‚   â”œâ”€â”€ app/dashboard/portfolio/  # portfolio editor page
â”‚   â”œâ”€â”€ app/dashboard/curator/    # curator editor + gig composer/list + series management (sub-project 3)
â”‚   â”œâ”€â”€ app/u/[handle]/           # server-rendered (SSR) public portfolio/curator page + open gigs
â”‚   â”œâ”€â”€ src/curator/CuratorForms.tsx # curator editor form sections (sub-project 3)
â”‚   â””â”€â”€ src/gigs/GigForms.tsx     # gig composer + series form sections (sub-project 3)
â””â”€â”€ tests-rules/                  # Firestore + Storage security rules emulator tests
```

`packages/shared` owns every cross-boundary type and validation rule â€” functions and both apps
...
- **`pnpm --filter @gatekeep/web exec next typegen`** â€” run once per fresh clone. It generates the
...
- **Java (JRE/JDK) 11+ on `PATH`**, required by the Firebase Emulator Suite (Firestore emulator
  ...
  `~\.jre\jdk-21â€¦` and `~\.jdks\jdk-21â€¦`). Example, used before every emulator command below:
  ```bash
  # bash
...
npx expo start                        # from apps/mobile: Expo dev server (use a dev build â€” Expo Go
                                       # cannot do native Google/Apple sign-in)

pnpm --filter @gatekeep/web lint      # ESLint (apps/web)
pnpm --filter @gatekeep/mobile lint   # ESLint (apps/mobile) â€” green as of sub-project 2 (see Task 15
...
**Troubleshooting â€” functions "not found" from the emulator:** `pnpm emu` (and `emu:test`/
`emu:rules`) can print `All emulators ready!` and still leave one or more functions unregistered
if the Functions emulator's discovery step â€” which has to `require()` every module under
`functions/src`, including the native `sharp`/`ffmpeg-static`/`ffprobe-static` dependencies added
in sub-project 2 â€” takes longer than its default 30s window. The symptom is a working emulator UI
...
can create, always via a callable (`createGig`/`updateGig`/`publishGig`/`cancelGig`) â€” clients
...
`perSet`) â€” never floating-point dollars.

**Key flows.** Curator onboarding is the same wizard/review pipeline as musicians (draft â†’ submit
â†’ admin `reviewProfile` approve/reject), plus curator-specific required content (photos, location,
...
enables the underlying Cloud Scheduler job** â€” the emulator has no scheduler component, so
...
Each of the five steps runs in its own try/catch with its own chunked batch writer â€” a poisoned
...
**Concepts.** A `bookings/{id}` top-level doc is the one record of a musicianâ†”curator booking
relationship, created through either door â€” `applyToGig` (musician quotes on an `open` gig) or
`offerGig` (curator quotes to a musician) â€” and mutated only via callables (clients never write
`bookings`). Both doors produce the same shape: an embedded `thread: OfferEntry[]` (capped at 50
entries) carries structured counter-offers (amount, expected quantity, â‰¤280-char note â€” **terms
...
(`computeDepositCents`, integer cents, `Math.ceil`) as **data** (`status: "unpaid"` â€” sub-5 wires
real money), flips the gig `open â†’ filled` with `bookingId`/`bookedMusicianProfileId` linkage, and
auto-supersedes rival open bookings on the same gig. Cancellation windows are measured strictly
against gig start: curator cancels **< 72h** before start â†’ deposit forfeits to the musician (â‰¥72h
â†’ refunds); musician cancels **< 24h** before start â†’ refunds the deposit but appends an automatic
...
disputed mark (audit-preserving â€” the mark stays, just flagged `removedByAdmin`, never spliced).
...
public** â€” each rate structure (`perHour`/`perSong`/`perSet`) is independently `"curators"` or
...
â€” **this projection is the curator-facing surface**, readable by curatorAccess holders + members +
admins) and `ProfileDoc.publicBooking` (the preferences object, only when `preferences: "public"`,
else `null` â€” rendered on the public portfolio page, rates never appear here regardless of
...
Deposits and cancellation windows are **per occurrence** â€” `cancelOccurrence` pulls one date off a
...
**Shows sections are now live** â€” the SP2 "platform events only" contract fulfilled: both public
...
client-side filters, by design â€” sub-project 8 (full search: text search, ranking, maps, saved
...
`expectedQuantity`, `expectedTotalCents`) â€” sub-5 recomputes overtime for `perHour` and a
...
`MAX_OCCURRENCE_CANCELLATIONS`=100) are themselves settlement inputs â€” a full 100-entry array is a
...
`gigs.ts`'s `takedownGig` occurrence scope) settles as **not performed** â€” the completion sweep
...
(`deposit.forfeitedTo: "musician"`) â€” whether the platform carves out a fee from that forfeiture is
...
- **Materializer birth-decision race**: a `cancelBooking` landing between the sweep's step-1
  ...
  non-`confirmed` booking, with no reconciling sweep step today â€” accepted at v1 given the low
...
- **`functions/test/` helper duplication**: `bookings.test.ts`, `bookingLifecycle.test.ts`, and
  ...
  `seedSeries` rather than sharing them from `functions/test/helpers.ts` â€” worth consolidating the
  next time one of those files is touched.

## Environment variables

None are required for local development against the emulators â€” everything below is unset (empty
...
| `GEOCODER_PROVIDER` | functions | set to `google` to geocode gig/curator addresses via the real Google Geocoding API (`functions/src/geocode.ts`'s `getGeocoder()`) | unset/any other value â†’ `StubGeocoder`, a deterministic dev/test-only hash-based geocoder with a US-centric bounding box â€” **launch item**, see checklist below |
...
var â€” the v2 Cloud Functions mechanism that actually makes a secret available at invocation time in
...
2. Deploy â€” every onCall that can trigger a geocode (`updateCuratorProfile`, `createGig`/
...
a bare `process.env.GEOCODER_API_KEY` read in that case â€” set it via a `functions/.env` file (or
...
(`geocodeBudgets/{uid}`, `functions/src/geocode.ts`'s `consumeGeocodeBudget`) â€” 50 geocode calls per
...
again (the geocoded location is reused as-is via the stored `geocodedFrom` string) â€” only a
...
against the local emulators instead of real Firebase â€” `apps/web/src/lib/firebase-server.ts`'s
...
These are tracked gaps, not bugs â€” the app is unblocked without them, but they must be done
before a real launch:

- **Firebase console â†’ Authentication â†’ Sign-in method**: enable Email/Password, Google, and
  Apple providers on the `gatekeep-dev-jg` project (and again on whatever project id production
  uses).
- **Firebase console â†’ App Check**: register the web app with **reCAPTCHA v3** (produces the
  ...
  **and Cloud Storage** until both stores' builds exist, then flip to **enforce** as a
  launch-checklist item â€” **Storage must not be flipped to enforce before native mobile App Check
  ships**: mobile has no App Check attestation client code today, and enforcing early would lock
  the app out of its own uploads. Mobile ships v1 in monitor mode via the console only â€” no native
  ...
  plan of landing them there) and now live in a dedicated launch-prep track (SP2 spec Â§1), on the
  same must-review-before-launch list as the sub-project 1 admin/internal deferred items below.
- **`apps/mobile/src/auth/config.ts` â€” `GOOGLE_WEB_CLIENT_ID`**: currently a
  `REPLACE_FROM_FIREBASE_CONSOLE...` placeholder. Get the real web client ID from Firebase console
  â†’ Authentication â†’ Sign-in method â†’ Google â†’ Web SDK configuration, and set it there before
...
- **Sentry DSNs**: no Sentry account/project exists yet. Create one free project per app (or one
  ...
  with these unset â€” crash reporting is simply inert until then.
...
- **EAS build setup (in progress, 2026-08-27)**: `apps/mobile/eas.json` (development/preview/
  ...
  manual: `eas login` + `eas init` against the org account; Firebase console â†’ add an **Android
  app** (package `com.gatekeepbeta.app`) â†’ download `google-services.json` into `apps/mobile/` and add
  the EAS keystore's SHA-1 (`eas credentials`) to it (Google Sign-In fails on-device without it);
  add an **iOS app** (bundle `com.gatekeepbeta.app`) â†’ `GoogleService-Info.plist` likewise; then set
  ...
  exists (2026-08-27); keys live outside the repo â€” sub-5 wires `sk_test` via `defineSecret()`
...
- **Deploy-time `workspace:*` resolution**: `functions/package.json` depends on
  ...
  than failing or publishing a broken reference) â€” this hasn't been exercised yet, only
...
- **App Check enforcement is two changes, not one**: flipping Firestore/Functions App Check from
  ...
  unattested requests â€” each `onCall` also needs `enforceAppCheck: true` in its handler options
  ...
  second step â€” its enforcement toggle applies directly â€” but it still can't flip until native
...
- **Content takedown is a two-step for a full scrub**: an admin unpublishing an approved profile
  ...
  discovery â€” the public page 404s and Storage listing is denied â€” but deliberately leaves the
  ...
  `getDownloadURL` obtained while the profile was live still resolves â€” do both promptly.
- **Firebase Email Enumeration Protection**: confirm this is enabled (Firebase console â†’
  Authentication â†’ Settings) on both the `gatekeep-dev-jg` project and whatever project id
  production uses. The app's sign-in error handling degrades gracefully either way, but it
  currently assumes the protection is on rather than asserting it â€” verify the console toggle
  before launch instead of relying on that assumption.
- **Storage `staging/` lifecycle rule â€” LAUNCH BLOCKER**: configure a 24h TTL lifecycle rule on
  the `staging/` prefix of the production Storage bucket (Cloud Console / `gcloud storage buckets
  update --lifecycle-file`, not the Firebase console rules editor â€” this is a bucket-level GCS
  ...
  objects `processUpload` fails to clean up because its trigger never fired at all â€” see the
...
- ~~**Abandoned `processing`-track reaper**~~ â€” **DONE (sub-project 3):** a track created via
...
- **`PUBLIC_PROFILE_HOST`** (`apps/mobile/app/(musician)/portfolio.tsx`): still the
  ...
  hidden (`PUBLIC_PROFILE_HOST_READY`) rather than pointing at a dead URL â€” update the constant to
...
- **Geocoder provider**: set `GEOCODER_PROVIDER=google` on the production functions deployment, and
  ...
  a plain env var) before launch â€” see "Geocoder secret setup" above. Without it, every gig/curator
  address silently geocodes through `StubGeocoder` â€” a deterministic hash with a US-centric
...
- **Geocode budget ceiling**: the 50/day per-uid `geocodeBudgets/{uid}` ceiling (see "Geocoder
  ...
  not an env var â€” revisit the number if real curator usage patterns turn out to need more than 50
...
- **Cloud Scheduler enablement**: `functions/src/scheduled.ts`'s `dailySweep` only starts actually
  running once `firebase deploy` provisions the underlying Cloud Scheduler job â€” there is nothing
  to enable by hand, but the first production deploy should be followed by a Cloud Console check
  (Cloud Scheduler â†’ confirm the job exists and its next-run time looks right) since a silently
...
- **Verify the 5 sub-project 3 composite indexes on first deploy**: the Firestore emulator does not
  ...
  real project (Firebase console â†’ Firestore â†’ Indexes, or `firebase deploy --only
...
- **`LAUNCH_TIMEZONE`** (`@gatekeep/shared`'s `packages/shared/src/types.ts`): currently
  ...
  the actual launch metro's IANA zone before launch** â€” per-venue timezone support is a documented
...
- **UTC recurrence-time caveat**: a `gigSeries`' `recurrence.weekday`/`hour`/`minute` (and its
  ...
  (`functions/src/scheduled.ts`), NOT in `LAUNCH_TIMEZONE` or any curator's local time â€” a curator
  ...
  `apps/mobile/src/gigs/GigForms.tsx`) discloses this in-form ("Times are in UTC for now â€”
...
- **Name-search backfill (one-shot)**: `searchUsersByName`'s prefix query depends on every
  ...
  users â€” until then, name search simply won't find pre-existing accounts (the email-exact lookup
  path still works for them).
- **Device pass (mobile, before launch)** â€” verify on a real dev-build device, not just the
  simulator/emulator: (1) **Hermes ICU date formatting** â€” `apps/mobile/src/gigs/GigForms.tsx`'s
  ...
  the device's raw local-time string. (2) **Nested "events" Stack headers** â€” the curator tab's
  `app/(curator)/events/_layout.tsx` nests an Expo Router `Stack` (list â†’ composer â†’ gig detail â†’
  series detail) inside the outer `Tabs` navigator; confirm header/back-button behavior looks right
  across that nesting on both iOS and Android. (3) **Native provider sign-in** â€” carried over from
...
- **`backfillBookingVisibility` (one-shot) â€” CRITICAL ordering, must ship in the SAME release as
  the rules deploy**: `firestore.rules`' sub-project 4 tighten removes curators' old blanket read
  of `profiles/{id}/private/booking` (see "Booking flow" above) â€” curators from then on read only
  ...
  `backfillBookingVisibility` in the same release** â€” if the rules land first without the backfill
  ...
  the same way as `backfillDisplayNameLower` above â€” it's idempotent (a profile whose
...
- **New composite indexes deploy with `firebase deploy`**: sub-project 4 adds 16 composite indexes
  to `firestore.indexes.json` (7 more `gigs` composites â€” booked-musician/series/booking linkage
  queries â€” plus 9 new `bookings` composites, since `bookings` is itself a new collection this
  ...
  about them â€” confirm they actually build on the real project after the first production deploy
  (Firebase console â†’ Firestore â†’ Indexes, or `firebase deploy --only firestore:indexes` then watch
...
Smaller items from the sub-project 2 quality-review rounds â€” recorded in full in
...
- **Editor/admin polish** (web, Task 11): `Promise.allSettled` (not `Promise.all`) for the admin
  ...
  and throws `failed-precondition` otherwise) â€” not just a client-side UI convention as earlier
...
- **`ProfileContext`** (mobile, Task 14): `switchTo`'s caller-supplied `ProfileSummary` (used by
  ...
  â€” not a live bug today, but a footgun for the next caller of `switchTo`. Separately, the
  ...
  `baseline`/`lastProfileId`) â€” revisit if that inconsistency ever causes a real bug.

## Design docs

**Sub-project 1: Foundation** â€” `docs/superpowers/specs/2026-08-24-foundation-design.md` for the
full design spec and `docs/superpowers/plans/2026-08-24-foundation.md` for the task-by-task
implementation plan.

**Sub-project 2: Musician Portfolio** â€” `docs/superpowers/specs/2026-08-25-musician-portfolio-design.md`
for the full design spec and `docs/superpowers/plans/2026-08-25-musician-portfolio.md` for the
task-by-task implementation plan.

**Sub-project 3: Curator Profiles & Gig Postings** â€” `docs/superpowers/specs/2026-08-26-curator-gigs-design.md`
...
**Sub-project 4: Booking Flow** â€” `docs/superpowers/specs/2026-08-26-booking-flow-design.md` for
