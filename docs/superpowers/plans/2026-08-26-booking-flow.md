...
9. **(Added by Tasks 10/11 reviews — cosmetic sweep):** two stale comments (apps/web/app/dashboard/bookings/[bookingId]/page.tsx:9 and apps/web/src/bookings/CancelDialog.tsx:42 reference pre-extraction file locations); web gig-display helper consolidation (gigDisplay.ts vs GigForms.tsx formatGigDateTime + BookingForms.tsx gigLocationLabel — client files CAN import the plain gigDisplay.ts module, flip the import direction to reach one copy each); MusicianProfile empty-publicBooking edge (all four displayed fields null → bare heading; gate the section on at least one non-null); curator ShowCard could add the public-precision location line (public page, roaming planner gigs); ProfileBookingsList re-cap merged array at 25; web test-helper dedup note from Task 7 stands. **From Task 12's review:** mobile MusicianBrowse offer-success state should deep-link "View the booking thread →" with the returned bookingId (web parity); mobile GigBrowse date TextInputs re-query on every keystroke (re-query only when the parsed boundary changes); reason TextInputs get accessibilityLabel (thread report + cancel dialog); web MusicianProfile gets the publicBooking ≥1-non-null gate mobile already ships.
