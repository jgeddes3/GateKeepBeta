...
// each doc it converges — the sole writer of both projections, per the
// comments left in profiles.ts / firestore.rules by earlier tasks.
export async function rebuildBookingProjections(profileId: string): Promise<void> {
  ...
  const [bookingSnap, reliabilitySnap] = await Promise.all([bookingRef.get(), reliabilityRef.get()]);
...
export const backfillBookingVisibility = onCall<Record<string, never>>(
  ...
  async (req) => {
    ...
    await writeAudit({
      actorUid, action: "booking_visibility_backfilled", targetId: "*", detail: String(converged),
    });
    return { converged };
  },
);
