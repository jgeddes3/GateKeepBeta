...
// The full suite (pnpm emu:test) shares one emulator database across 48
// files: other suites leave hundreds of published events and approved "The
// Act" musicians behind, so an unscoped query (or the shared fixture title)
// does not reliably land this test's row on page 0 of a 20-per-page result
// (candidates cap at 300, ranked by soonness or follower count). A random
// token folded into the title/name and echoed back as `q` narrows the
// candidate set to just this test's row, regardless of what else is in the
// shared database.
const uniqueToken = (tag: string) => `zq${Date.now()}${Math.floor(Math.random() * 1e6)}${tag}`;
...
describe("search callable", () => {
  ...
  it("finds a published show by title words with AND semantics and returns only result fields", async () => {
    ...
    const token = uniqueToken("a");
    await adb.doc(`events/${eventId}`).update({ title: `${token} Owls` });
    await waitForIndex(`show_${eventId}`, (x) => x?.words.includes(token) ?? false);
    const fan = await makeFan("se2f");
    const hit = await callFn<SearchInput, SearchOutput>("search", input({ q: token }), fan.user);
    ...
    const miss = await callFn<SearchInput, SearchOutput>("search", input({ q: `${token} zzzqqq` }), fan.user);
  ...
  it("applies fan filters: free only, genres, when, and near me with distance", async () => {
    ...
    const token = uniqueToken("b");
    await adb.doc(`events/${eventId}`).update({ title: `${token} Owls` });
    const idx = await waitForIndex(`show_${eventId}`, (x) => x?.words.includes(token) ?? false);
    const fan = await makeFan("se3f");
    const free = await callFn<SearchInput, SearchOutput>("search", input({ q: token, filters: { freeOnly: true } }), fan.user);
    ...
    if (idx!.geo) {
      ...
      const far = await callFn<SearchInput, SearchOutput>("search", input({ q: token, filters: { nearMe: true }, location: { lat: idx!.geo.lat + 5, lng: idx!.geo.lng } }), fan.user);
  ...
  it("serves the curator face with act size, city, audio, and availability filters", async () => {
    ...
    await adb.doc(`profiles/${m.profileId}`).update({
      name: `${token} Act`, "portfolio.location": { city: "Austin", geo: null, geocodedFrom: "Austin" }, updatedAt: Date.now(),
    });
    await waitForIndex(`artist_${m.profileId}`, (x) => x?.actSize === "duo" && x?.cityLower === "austin" && (x?.words.includes(token) ?? false));
    const c = await makeApprovedCuratorProfile("se4c");
    const q = (filters: SearchInput["filters"]) => callFn<SearchInput, SearchOutput>("search", input({ face: "curator", q: token, filters }), c.owner.user);
